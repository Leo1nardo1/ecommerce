<div>
  <div class="w-full max-w-[85rem] py-10 px-4 sm:px-6 lg:px-8 mx-auto">
    <h1 class="text-4xl font-bold text-slate-500 dark:text-white">My Orders</h1>
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              ...
              <tbody>
                ...
                if($order->status_payment == 'pending') {
                  $status_payment = '<span class="bg-blue-500 py-1 px-3 rounded text-white shadow">Pending</span>';
                }
                if($order->status_payment == 'failed') {
                  $status_payment = '<span class="bg-red-500 py-1 px-3 rounded text-white shadow">Failed</span>';
                }
                if($order->status_payment == 'paid') {
                  $status_payment = '<span class="bg-green-500 py-1 px-3 rounded text-white shadow">Paid</span>';
                }
                ...
                <tr class="odd:bg-white even:bg-gray-100 dark:odd:bg-slate-900 dark:even:bg-slate-800" wire:key='{{ $order->id }}'>
                  ...
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200">{!! $status_payment !!}</td>
